create or replace  table scratch.riders.last_90_dys_metrics as
 ...
 ,master_table as
    ...
    *, row_number () over (partition by zone_code, city_name, order_month order by order_cnt desc)as rnk
    ...
    group by 1,2,3
     ...
     ,a.city_name
     ,a.order_month
     ,max(b.top_1_cuisine) as top_1_cuisine_last_90_dys
     ,max(b.top_2_cuisine) as top_2_cuisine_last_90_dys
     ,max(b.top_3_cuisine) as top_3_cuisine_last_90_dys
     
     ,count(distinct ID)as order_cnt_last_90_dys
     ,avg (AOD)as avg_AOD_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY AOD)as p10_AOD_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY AOD)as p20_AOD_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY AOD)as p50_AOD_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY AOD)as p70_AOD_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY AOD)as p90_AOD_last_90_dys
  
     ,avg(EOD)as avg_EOD_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY EOD)as p10_EOD_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY EOD)as p20_EOD_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY EOD)as p50_EOD_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY EOD)as p70_EOD_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY EOD)as p90_EOD_last_90_dys


      ,avg(ERAT)as avg_ERAT_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY ERAT)as p10_ERAT_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY ERAT)as p20_ERAT_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY ERAT)as p50_ERAT_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY ERAT)as p70_ERAT_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY ERAT)as p90_ERAT_last_90_dys
     
        
     ,avg(RIDER_TO_RESTAURANT_MINS)as avg_RIDER_TO_RESTAURANT_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY RIDER_TO_RESTAURANT_MINS)as p10_RIDER_TO_RESTAURANT_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY RIDER_TO_RESTAURANT_MINS)as p20_RIDER_TO_RESTAURANT_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY RIDER_TO_RESTAURANT_MINS)as p50_RIDER_TO_RESTAURANT_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY RIDER_TO_RESTAURANT_MINS)as p70_RIDER_TO_RESTAURANT_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY RIDER_TO_RESTAURANT_MINS)as p90_RIDER_TO_RESTAURANT_MINS_last_90_dys

     ,avg(FPT)as avg_FPT_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY FPT)as p10_FPT_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY FPT)as p20_FPT_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY FPT)as p50_FPT_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY FPT)as p70_FPT_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY FPT)as p90_FPT_last_90_dys

     ,avg(RESTO_CUSTOMER_MINS) as avg_RESTO_CUSTOMER_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY RESTO_CUSTOMER_MINS)as p10_RESTO_CUSTOMER_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY RESTO_CUSTOMER_MINS)as p20_RESTO_CUSTOMER_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY RESTO_CUSTOMER_MINS)as p50_RESTO_CUSTOMER_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY RESTO_CUSTOMER_MINS)as p70_RESTO_CUSTOMER_MINS_last_90_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY RESTO_CUSTOMER_MINS)as p90_RESTO_CUSTOMER_MINS_last_90_dys
 
     ,avg(WAIT_AT_CUSTOMER)as avg_WAIT_AT_CUSTOMER_last_90_dys
     ,avg(LATENESS)as avg_lateness_last_90_dys
     ,sum(LATE_BY_5_MINS)/ count(distinct ID)as late_by_5_mins_perc_last_90_dys
     ,sum(LATE_BY_10_MINS)/ count(distinct ID)as late_by_10_mins_perc_last_90_dys
     ,sum(LATE_BY_15_MINS)/ count(distinct ID)as late_by_15_mins_perc_last_90_dys
     ,avg(TEMPERATURE)as avg_TEMPERATURE_last_90_dys
     ,avg(WIND_SPEED)as avg_WIND_SPEED_last_90_dys
     ,sum (CROSS_ZONE_PICKUP_FLAG)/ count(distinct ID)as perc_cross_zone_pickup_last_90_dys
     ,sum (CROSS_ZONE_DLVRY_FLAG)/ count(distinct ID)as perc_cross_zone_dlvry_last_90_dys
